...
def sound_handler(channel_num, volume,sound_effect):
	...
	print(channel_num, sound_effect)
	return(channel_num)
...
def ffmpeg_capture(camera_index, window_name,shared_frame):
	...
	X_MIN,X_MAX,Y_MIN,Y_MAX=update_crop(camera_index,ZOOM_M,X_DIM,Y_DIM)#set crop
	...
	with sp.Popen(command, stdout=sp.PIPE, bufsize=10**5,stderr=sp.DEVNULL) as pipe:
		while True:
			...
			if using_double:
				...
				shared_frame["im"]=cv2.resize(frame2,(320,240))
...
if __name__=="__main__":
	...
	while (run_main_loop):
		if (time.time()-print_time_m)>1:#print out player info
			...
			try:
				print("T: ",f"{(t_bottom_while_loop-t_before_player_logic):.3f}",f"{(t_before_recieve-t_before_player_logic):.3f}",f"{(t_before_send-t_before_recieve):.3f}",f"{(t_bottom_while_loop-t_before_send):.3f}")
			except:
				pass
			print_time_m=time.time()
		...
		if health<=0:#check if dead
			...
			dead=False
		prev_switch_state=switch_on
		if serial_arr[0]==1:	#switch on or off
			switch_on=True
		else:
			switch_on=False
		t2=time.time()
